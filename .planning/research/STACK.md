...
**Domain:** Lightweight production-ready semantic retrieval API for structured TCM classics data
**Researched:** 2026-06-14
**Confidence:** HIGH for Python/FastAPI/Qdrant/BM25 ingestion stack; MEDIUM for embedding/reranker provider until customer confirms API account and deployment constraints

## Recommendation

Build v1 as a Python retrieval microservice, not as a LangChain-style chatbot app.

Use FastAPI for HTTP APIs, pandas/openpyxl for Excel ingestion, Qdrant for vector storage and hybrid dense+sparse retrieval, `bm25s`/`jieba` as a local lexical baseline, and an external Chinese-capable embedding API for the 2-3 week demo. Add reranking as an optional second-stage module behind a feature flag. Keep MySQL as an integration source owned by the Java/business backend, not as the retrieval engine.

This stack fits the actual product shape: about 1,248 structured rows in v1, maybe hundreds of thousands later, stable management APIs, deterministic search results, and fast demo deployment. It avoids spending the first milestone building LLM conversation infrastructure that the project explicitly moved out of v1.
...
Recommended initial Python dependencies:

```bash
# Runtime
uv add fastapi==0.136.3 "uvicorn[standard]==0.49.0" pydantic==2.13.4 pydantic-settings==2.14.1
uv add qdrant-client==1.18.0 pandas==3.0.3 openpyxl==3.1.5 bm25s==0.3.9 jieba==0.42.1
uv add httpx==0.28.1 orjson==3.11.9 typer==0.26.7

# Optional provider clients; add only when selected
uv add openai==2.41.1
uv add FlagEmbedding==1.4.0 sentence-transformers==5.5.1
...
1. Parse Excel with pandas/openpyxl, starting from the true header/data rows confirmed in the sample workbook.
2. Normalize every row into a stable internal record:
   - `record_id`: generated stable ID, not raw Excel row number alone.
   - `formula_code`: business formula code if supplied; otherwise nullable.
   - `formula_names`: parsed list because one source cell may mention multiple formulas.
   - `search_text`: weighted concatenation of symptoms, aliases, tongue, pulse, pattern, disease, etiology, treatment method, and original text.
   - `display_payload`: full original fields needed by frontend/Java backend.
3. Build lexical index with `bm25s` + jieba tokens.
4. Generate dense embeddings for `search_text` and upsert to Qdrant with payload.
5. At query time:
   - Build query text from patient symptoms, tongue, pulse, and extra description.
   - Retrieve dense candidates from Qdrant.
   - Retrieve BM25 candidates locally or via Qdrant sparse vectors.
   - Fuse scores with Reciprocal Rank Fusion or weighted normalized ranks.
   - Optionally rerank top 30-50 candidates.
   - Return topK with ranking score, not a medical confidence score.

Default v1 scoring recommendation:
...
**If the customer requires China-hosted or non-OpenAI model access:**
...
**If no external API keys are available for the demo:**
...
**If the corpus expands to 200 books with consistent fields:**
- Keep Qdrant.
...
**If each book has different fields:**
...
---
*Stack research for: 中医方剂检索系统 retrieval API*
*Researched: 2026-06-14*
